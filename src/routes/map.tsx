...
const districtLabel = (t: DistrictType, id: string) =>
  t === "congressional" ? id : `District ${id}`;
...
function weightNote(
  ...
  const pct =
    pctVal != null ? ` · ${pctVal > 0 && pctVal < 0.1 ? "<0.1" : pctVal.toFixed(1)}%` : "";
      ...
      <div className="relative border border-border rounded-sm overflow-hidden bg-[#050505]">
        <svg viewBox="0 0 960 540" className="relative z-[5] w-full h-auto block">
          ...
          <g fontFamily="var(--font-mono)" fontSize="9" fill="rgba(148,163,184,0.6)">
            ...
            />
            <line
              x1={940 - Math.round(px500km / 2)}
              y1={513}
              x2={940 - Math.round(px500km / 2)}
              y2={519}
              stroke="rgba(148,163,184,0.6)"
              strokeWidth="1"
            />
            <line
              ...
              strokeWidth="1"
            />
            <text x={940 - px500km} y={508} textAnchor="middle">
              0
            </text>
            <text x={940} y={508} textAnchor="middle">
              500 KM
            </text>
          </g>
        </svg>
        ...
        <div className="absolute bottom-8 left-3 z-10 pointer-events-none hidden sm:block border border-border/60 bg-black/60 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground space-y-1">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-block h-2 w-3 bg-amber/50 border border-amber" /> LIVE ·
            SELECTABLE
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-block h-2 w-3 bg-[#141a26] border border-border" /> OFFLINE ·
            NOT LOADED
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-block h-px w-3 bg-muted-foreground/50" /> STATE BORDER
          </div>
        </div>
  ...
  return (
    <div className="border border-border bg-surface rounded-sm p-8 text-center">
      ...
      <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">
        Only Florida is loaded right now. Other states will light up as their public records are
        ingested.
      </p>
    </div>
  );
...
function FloridaFinder({ onBack }: { onBack: () => void }) {
  ...
  const groups = useMemo(() => {
    const pick = (level: string, chamber: string) =>
      rows
        .filter((r) => r.level === level && r.chamber === chamber)
        .sort((a, b) => distNum(a.district) - distNum(b.district));
    return {
  ...
  const repFor = (type: DistrictType, districtId: string) => {
    ...
    return rows.find(
      (r) => r.level === m.level && r.chamber === m.chamber && r.district === districtId,
    );
  };
  ...
  const zipNotFound =
    !!submittedZip && !zipQuery.isLoading && !zipQuery.isError && zipRows.length === 0;
    ...
    <>
      ...
      <div className="mt-3 border border-cyan/40 bg-cyan/5 rounded-sm p-3">
        <p className="text-sm text-foreground leading-snug">
          A ZIP can span several districts — we show <span className="text-cyan">all of them</span>,
          weighted by land-area overlap (U.S. Census data), and never guess a single one. Some
          PO-box/business ZIPs have no boundary and won't resolve — pick your district instead if
          so.
        </p>
      </div>
      ...
      {!supabaseConfigured ? (
        ...
        <ResultsPanel
          ...
          districtResults={zipRows.map((r) => ({
            type: r.district_type,
            district_id: r.district_id,
            weight: r.area_weight,
          }))}
        ...
        <>
          {zipNotFound && (
            <div className="mt-4 border border-amber/50 bg-amber/5 rounded-sm p-4">
              ...
              <p className="mt-1 text-sm text-foreground">
                That ZIP has no district boundary in the Census crosswalk (often a PO-box or
                business-only ZIP). Pick your district below instead.
              </p>
            </div>
          )}
          ...
          <div className="mt-4">
            <div className="mono-label text-cyan mb-2">
              {zipNotFound ? "PICK YOUR DISTRICT INSTEAD" : "OR PICK YOUR DISTRICT"}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <DistrictSelect
                ...
                value={senate}
                onChange={setSenate}
                options={groups.senate}
                render={(r) => `Dist ${r.district} — ${r.full_name}`}
              />
              <DistrictSelect
                label="State House District"
                value={house}
                onChange={setHouse}
                options={groups.house}
                render={(r) => `Dist ${r.district} — ${r.full_name}`}
              />
              <DistrictSelect
                ...
                value={usHouse}
                onChange={setUsHouse}
                options={groups.usHouse}
                render={(r) => `${r.district} — ${r.full_name}`}
                valueKey={(r) => r.district ?? ""}
              />
            </div>
          </div>
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mono-label text-muted-foreground mb-2">
        U.S. SENATE · STATEWIDE (represents all of Florida)
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {senators.map((r) => (
          <RepCard key={r.ocd_person_id} rep={r} note="U.S. Senator" />
        ))}
      </div>
      ...
      {TYPE_ORDER.map((type) => {
        ...
        return (
          <div key={type}>
            <div className="mono-label text-muted-foreground mt-5 mb-2">
              {TYPE_META[type].label.toUpperCase()}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {items.map((d, i) => {
  ...
  return (
    <div className="border border-dashed border-amber/50 bg-amber/5 rounded-sm p-4">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {TYPE_META[type].label} · no current member for this district.
      </p>
    </div>
  );
}
...
function repScope(rep: RepIndexRow): string | null {
  if (!rep.district) return null;
  if (rep.level === "federal") return `federal:US:${rep.district}`;
  if (rep.level === "state") return `state:FL:${rep.district}`;
  return null;
}

function RepCard({ rep, note }: { rep: RepIndexRow; note: string }) {
  const party = toPartyCode(rep.party);
  const scope = repScope(rep);
  return (
    <div className="border border-border bg-surface rounded-sm p-4 transition hover:border-amber/40">
      <div className="mono-label text-cyan mb-2">{note}</div>
      <Link
        to="/officials/$id"
      ...
      >
        <OfficialAvatar
          official={{ name: rep.full_name, photoSeed: rep.ocd_person_id, party }}
          size={44}
        />
        <div className="min-w-0 flex-1">
          ...
          <OfficeTag
            level={levelOf(rep.level)}
            text={rep.district ? `Dist ${rep.district}` : (rep.office ?? "—")}
          />
      ...
      {scope && (
        <div className="mt-3 pt-3 border-t border-border/60">
          <Link
            to="/discuss"
            search={{ scope }}
            className="inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-widest text-muted-foreground hover:text-amber transition"
          >
